...
function preSubmit() {
	...
	if (missingFields == "") {
		...
		// for (var i = 0; i < taskNumber.value; i++) {
		// if (document.getElementById(collectionTypeId).value !=
		// "JOB_PARAMETRIC") {
...
//		executable.value = "echo";
		...
		var arguments = [[20,0,20,1500],[20,0,20,2000],[20,0,10,1500],[20,0,10,2000],[20,0,10,3000]];

		for (var i = 0; i < arguments.length; i++) {
			for (var j = 0; j < arguments[i].length; j++) {
				document.getElementById('argument_' + (i+1) + '_' + (j+1)).value = arguments[i][j];
...
function updatePage() {
	...
	if (numInput >= 1 && numInput <= 10) {
		...
		alert("Specify task number between 1 and 10.")
	}
}
