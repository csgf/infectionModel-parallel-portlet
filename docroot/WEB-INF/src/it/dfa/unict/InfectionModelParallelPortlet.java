...
public class InfectionModelParallelPortlet extends MVCPortlet {


private final Log _log = LogFactoryUtil.getLog(InfectionModelParallelPortlet.class);
...
//	}
	...
	public void submit(ActionRequest actionRequest,
		...
		if (arguments != null && !arguments.isEmpty()) {
			...
			try {
				JSONArray jsonArray = JSONFactoryUtil.createJSONArray(arguments);
				for (int i = 0; i < jsonArray.length(); i++) {
					JSONArray jsonArg = jsonArray.getJSONArray(i);
					String[] args = new String[jsonArg.length()];
					for (int j = 0; j < jsonArg.length(); j++) {
						args[j] = jsonArg.getString(j);
					}
					tmp.add(args);
				}
...
//			}
			if(appInput.getArguments()!=null && appInput.getArguments().size() > 0 && i < appInput.getArguments().size()){
				String[] tmp = appInput.getArguments().get(i);
				String args = "";
				for (String s : tmp) {
					args += s + " ";
				}
